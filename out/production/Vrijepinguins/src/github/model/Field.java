...
public class Field
{
    ...
    public List<AreaLocation> adjacentAreaLocations(AreaLocation areaLocation)
    {
        assert areaLocation != null : "Null location passed to adjacentLocations";
        // The list of locations to be returned.
        List<AreaLocation> areaLocations = new LinkedList<AreaLocation>();
        if(areaLocation != null) {
            int row = areaLocation.getRow();
            int col = areaLocation.getCol();
            for(int roffset = -1; roffset <= 1; roffset++) {
                int nextRow = row + roffset;
                if(nextRow >= 0 && nextRow < depth) {
                    for(int coffset = -1; coffset <= 1; coffset++) {
                        ...
                        // Exclude invalid locations and the original location.
                        if(nextCol >= 0 && nextCol < width && (roffset != 0 || coffset != 0)) {
                            areaLocations.add(new AreaLocation(nextRow, nextCol));
                        }
                    }
                }
            }
            ...
            // Shuffle the list. Several other methods rely on the list
            // being in a random order.
            Collections.shuffle(areaLocations, rand);
        }
